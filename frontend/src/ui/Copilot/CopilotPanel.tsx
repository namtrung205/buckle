...
import { Close, DeleteOutline, Refresh, Send, Settings, SmartToy, Stop, Undo } from '@mui/icons-material'
...
const defaultRateLimit = (provider: ProviderKind): RateLimitSettings => ({
  mode: 'auto', maxConcurrent: 2, rpm: provider === 'groq' ? 24 : null,
  tpm: provider === 'groq' ? 6400 : null, safetyFactor: .8, maxWaitSeconds: 30, maxRetries: 2,
})
...
const RateLimitFields = ({ value, onChange }: { value: RateLimitSettings; onChange: (next: RateLimitSettings) => void }) => {
  ...
  return <Stack spacing={1} sx={{ p: 1, border: `1px solid ${colors.border}`, borderRadius: 1 }}>
    <Typography fontSize={12} fontWeight={700}>Outbound rate limiting</Typography>
    <FormControl size="small" fullWidth><InputLabel>Mode</InputLabel><Select label="Mode" value={value.mode} onChange={event => onChange({ ...value, mode: event.target.value as RateLimitSettings['mode'] })}><MenuItem value="auto">Auto — learn provider headers</MenuItem><MenuItem value="manual">Manual</MenuItem><MenuItem value="disabled">Disabled</MenuItem></Select></FormControl>
    {value.mode !== 'disabled' && <>
      <Stack direction="row" spacing={1}><TextField size="small" type="number" label="Concurrent" value={value.maxConcurrent} inputProps={{ min: 1, max: 10 }} onChange={event => onChange({ ...value, maxConcurrent: number(event.target.value, 1) })} /><TextField size="small" type="number" label="Safety %" value={Math.round(value.safetyFactor * 100)} inputProps={{ min: 10, max: 100 }} onChange={event => onChange({ ...value, safetyFactor: number(event.target.value, 80) / 100 })} /></Stack>
      <Stack direction="row" spacing={1}><TextField size="small" type="number" label="RPM (optional)" value={value.rpm ?? ''} onChange={event => onChange({ ...value, rpm: optionalNumber(event.target.value) })} /><TextField size="small" type="number" label="TPM (optional)" value={value.tpm ?? ''} onChange={event => onChange({ ...value, tpm: optionalNumber(event.target.value) })} /></Stack>
      <Stack direction="row" spacing={1}><TextField size="small" type="number" label="Max queue wait (s)" value={value.maxWaitSeconds} inputProps={{ min: 0, max: 300 }} onChange={event => onChange({ ...value, maxWaitSeconds: number(event.target.value, 0) })} /><TextField size="small" type="number" label="429 retries" value={value.maxRetries} inputProps={{ min: 0, max: 5 }} onChange={event => onChange({ ...value, maxRetries: number(event.target.value, 0) })} /></Stack>
    </>}
  </Stack>
}
...
const CopilotPanel = observer(() => {
  ...
  const [connectionDraft, setConnectionDraft] = useState({ provider: 'deepseek' as ProviderKind, label: '', apiKey: '', baseUrl: '', modelIds: '', rateLimit: defaultRateLimit('deepseek') })
  ...
  return <Paper elevation={12} sx={{ position: 'fixed', right: 16, bottom: 36, zIndex: 1300, width: 430, height: 610, display: 'flex', flexDirection: 'column', bgcolor: colors.surface, border: `1px solid ${colors.border}`, overflow: 'hidden' }}>
    ...
    <Dialog open={settingsOpen} onClose={() => setSettingsOpen(false)} maxWidth="xs" fullWidth>
      <DialogTitle>AI provider connections</DialogTitle><DialogContent><Stack spacing={1.5} sx={{ pt: 1 }}>
        {connections.map(connection => <Box key={connection.id} sx={{ p: 1, bgcolor: colors.surfaceAlt, borderRadius: 1 }}><Stack direction="row" alignItems="center"><Box sx={{ flex: 1, minWidth: 0 }}><Typography fontSize={13} fontWeight={700}>{connection.label}</Typography><Typography fontSize={11} color={colors.textDim}>{connection.models.length} models · {connection.keyHint} · rate {connection.rateLimit.mode}</Typography></Box><Button size="small" onClick={() => setRateEditor({ connectionId: connection.id, value: { ...connection.rateLimit } })}>Tune</Button><IconButton size="small" onClick={() => void removeConnection(connection.id)}><DeleteOutline fontSize="small" /></IconButton></Stack>{rateEditor?.connectionId === connection.id && <Stack spacing={1} mt={1}><RateLimitFields value={rateEditor.value} onChange={value => setRateEditor({ connectionId: connection.id, value })} /><Stack direction="row" justifyContent="flex-end"><Button size="small" onClick={() => setRateEditor(null)}>Cancel</Button><Button size="small" variant="contained" onClick={() => void saveRateLimit()}>Save limits</Button></Stack></Stack>}</Box>)}
        <Typography fontSize={12} fontWeight={700}>Add connection</Typography>
        <FormControl size="small" fullWidth><InputLabel>Provider</InputLabel><Select label="Provider" value={connectionDraft.provider} onChange={event => { const provider = event.target.value as ProviderKind; setConnectionDraft(current => ({ ...current, provider, rateLimit: defaultRateLimit(provider) })) }}>{(['openai', 'deepseek', 'anthropic', 'gemini', 'openrouter', 'nvidia', 'groq', 'compatible'] as ProviderKind[]).map(value => <MenuItem key={value} value={value}>{providerLabel(value)}</MenuItem>)}</Select></FormControl>
        ...
        {connectionDraft.provider === 'compatible' && <TextField size="small" label="HTTPS base URL" value={connectionDraft.baseUrl} onChange={event => setConnectionDraft(current => ({ ...current, baseUrl: event.target.value }))} />}
        <TextField size="small" label="Model IDs (optional, comma-separated)" value={connectionDraft.modelIds} onChange={event => setConnectionDraft(current => ({ ...current, modelIds: event.target.value }))} />
        <RateLimitFields value={connectionDraft.rateLimit} onChange={rateLimit => setConnectionDraft(current => ({ ...current, rateLimit }))} />
        {connectionError && <Typography color="error" fontSize={12}>{connectionError}</Typography>}
        <Typography color={colors.textFaint} fontSize={11}>Keys and rate-limit policies stay in backend memory and are isolated by browser session.</Typography>
      </Stack></DialogContent><DialogActions><Button onClick={() => setSettingsOpen(false)}>Close</Button><Button variant="contained" disabled={!connectionDraft.apiKey.trim()} onClick={() => void saveConnection()}>Connect</Button></DialogActions>
    </Dialog>
  </Paper>
})
