...
const CopilotPanel = observer(() => {
  ...
  const retryMessage = (message: Message) => { if (!busy) void sendPrompt(message.text) }
  ...
  return <Paper elevation={12} sx={{ position: 'fixed', right: 16, bottom: 36, zIndex: 1300, width: 430, height: 610, display: 'flex', flexDirection: 'column', bgcolor: colors.surface, border: `1px solid ${colors.border}`, overflow: 'hidden' }}>
    ...
    <Box sx={{ flex: 1, overflowY: 'auto', p: 1.25, display: 'flex', flexDirection: 'column', gap: .8 }}>{messages.map(message => message.role === 'user' ? <Box key={message.id} sx={{ alignSelf: 'flex-end', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: .2, maxWidth: '88%' }}><Box sx={{ bgcolor: colors.accentHover, color: colors.text, px: 1.1, py: .7, borderRadius: 1, fontSize: 13, whiteSpace: 'pre-wrap', overflowWrap: 'anywhere' }}>{message.text}</Box><Button size="small" disabled={busy} onClick={() => retryMessage(message)} sx={{ fontSize: 10, minWidth: 0, px: .8, color: colors.textDim }}><Refresh sx={{ fontSize: 12, mr: .3 }} />Retry</Button></Box> : <Box key={message.id} sx={{ alignSelf: 'flex-start', maxWidth: message.role === 'activity' ? '100%' : '88%', bgcolor: message.role === 'activity' ? colors.bg : colors.surfaceAlt, border: message.role === 'activity' ? `1px solid ${colors.border}` : undefined, color: message.role === 'error' ? colors.danger : colors.text, px: 1.1, py: .7, borderRadius: 1, fontSize: message.role === 'activity' ? 11 : 13, whiteSpace: 'pre-wrap' }}>{message.text}{!!message.affected?.length && <Button size="small" sx={{ ml: 1, fontSize: 10 }} onClick={() => selectAffected(message.affected!)}>Select</Button>}</Box>)}
