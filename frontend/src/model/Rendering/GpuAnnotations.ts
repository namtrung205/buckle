...
/** Member end release flags parsed from the canonical `release` string.
 *  Supports both the UI value set (`fixed-pinned`, `pinned-fixed`,
 *  `pinned-pinned`) and the compact AI/script forms (`i`, `j`, `ij`). */
export const releaseEnds = (release: string | undefined | null): { start: boolean; end: boolean } => {
  switch (release) {
    case 'i':
    case 'pinned-fixed':
...
/** Symbol kind that paints a small filled green circle (member release pin). */
export const SYMBOL_KIND_RELEASE = 1
